...
typedef struct ms_say_something_t {
	...
	uint8_t* ms_some_string;
	size_t ms_len;
} ms_say_something_t;

typedef struct ms_say_something_twice_t {
	sgx_status_t ms_retval;
	uint8_t* ms_some_string;
	size_t ms_len;
} ms_say_something_twice_t;
...
static sgx_status_t SGX_CDECL sgx_say_something(void* pms)
{
	...
	uint8_t* _tmp_some_string = ms->ms_some_string;
	...
	if (_tmp_some_string != NULL && _len_some_string != 0) {
		...
		memcpy((void*)_in_some_string, _tmp_some_string, _len_some_string);
	}

	ms->ms_retval = say_something((const uint8_t*)_in_some_string, _tmp_len);
err:
	if (_in_some_string) free((void*)_in_some_string);

	return status;
}

static sgx_status_t SGX_CDECL sgx_say_something_twice(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_say_something_twice_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_say_something_twice_t* ms = SGX_CAST(ms_say_something_twice_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_some_string = ms->ms_some_string;
	size_t _tmp_len = ms->ms_len;
	size_t _len_some_string = _tmp_len;
	...
	ms->ms_retval = say_something_twice((const uint8_t*)_in_some_string, _tmp_len);
err:
	if (_in_some_string) free((void*)_in_some_string);
...
SGX_EXTERNC const struct {
	...
	5,
	{
		...
		{(void*)(uintptr_t)sgx_say_something_twice, 0},
